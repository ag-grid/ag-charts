 ...
 *
 * Usage:
 *   SONAR_TOKEN=<your-token> npx tsx external/prompts/commands/sonar-fix/scripts/mark-sonar-issues.ts --config path/to/config.json
 *
 *   # Dry run:
 *   DRY_RUN=true SONAR_TOKEN=<your-token> npx tsx external/prompts/commands/sonar-fix/scripts/mark-sonar-issues.ts --config path/to/config.json
 *
 * Prerequisites:
...
function loadBatchConfig(): BatchConfig[] {
    ...
    if (configIndex < 0 || !args[configIndex + 1]) {
        ...
        console.error('Usage:');
        console.error(
            '  npx tsx external/prompts/commands/sonar-fix/scripts/mark-sonar-issues.ts --config path/to/config.json'
        );
        console.error('');
