---
targets: ['*']
description: 'Review TypeScript API compatibility and breaking changes for releases'
---
...
When resolving branch names, always use the remote-tracking ref (e.g., `origin/b13.1.0`) rather than a bare branch name, to avoid merge-base failures when the local branch doesn't exist or has diverged.
...
This command MUST write its report to a file (see Output Location below). Use plan mode for the analysis phases (the deep review benefits from extended thinking), then **exit plan mode** to write the report file. Do not finish the command while still in plan mode — the results will be lost.
...
**Note on callback parameters:** Adding parameters to a callback type is NOT a hard break. TypeScript allows a function with fewer parameters to be assigned to a type expecting more (parameter bivariance). Users supplying inline callbacks will not get compiler errors. Only classify callback changes as hard breaks if the return type changes incompatibly, parameters are removed, or parameter types are narrowed.
