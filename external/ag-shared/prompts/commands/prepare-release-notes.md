---
targets: ['*']
description: 'Generate release notes files for a given version, release date, and features'
---
...
2.  **Create Working Branch:**
    - Pull the latest changes from `origin/latest`.
    - Get the user's initials from `git config user.name` (e.g., "Dan Goodman" → "dg", "John Smith" → "js").
    - Create a new branch named `<initials>/release-notes-<major>-<minor>` (e.g., `dg/release-notes-13-1`).
    - All changes will be made on this branch but **not committed** - leave them as unstaged changes for manual review.

3.  **Create Changelog File:**
    - Create or overwrite the file at `packages/ag-charts-website/public/changelog/releases/<major>_<minor>_<patch>.md` (e.g., `13_1_0.md`).
...
4.  **Update `releaseVersionNotes.json`:**
    - Read the file `packages/ag-charts-website/public/changelog/releaseVersionNotes.json`.
    - Parse the JSON content into an array.
    - Create a new JSON object: `{ "release version": "<version>", "markdown": "/releases/<major>_<minor>_<patch>.md" }`.
        - Example for version `13.1.0`: `{ "release version": "13.1.0", "markdown": "/releases/13_1_0.md" }`
...
5.  **Update `ag-charts-versions.json`:**
...
6.  **Create Upgrade Guide (for Major/Minor releases only):**
    ...
    - Replace <!-- XXXX --> in the blog link with the release version in the format `<major>-<minor>` for minor releases and `<major>` for major releases.
    - Ask the user which of these sections have actual content to fill in: Breaking Changes, Behaviour Changes, Removal of Deprecated APIs, Deprecations.
    - For each section with **no content**: uncomment the "nothing here" line by removing its surrounding `<!--` / `-->` comment markers. Leave the "Otherwise" expandingSection block still commented.
    - For each section **with content**: uncomment the `expandingSection` block by removing its surrounding `<!--` / `-->` comment markers, and add placeholder items (e.g. `- TODO: describe change`). Leave the "nothing here" line still commented.
    - Do NOT delete either block — both must remain in the file so it's easy to switch between them later.
    - The instructional `<!-- If ... -->` and `<!-- Otherwise -->` label comments should remain unchanged.
...
7.  **Update Migration `index.mdoc` (for Major releases only):**
...
8.  **Format and Finalise:**
    - Run `yarn nx format --all`.
    - **Do not commit the changes.** Leave them as unstaged changes for manual review by the user.
    - Provide a summary of the files created/modified.
