...
#   4. generates the repo's own gitignored .claude outputs (rules, local skills),
#      which the skipped postinstall would otherwise have produced
#   5. seeds $AG_CLOUD_CACHE_DIR with node_modules and the resolved node path, so
...
# ---------------------------------------------------------------------------
# The repo's own Claude Code config
# ---------------------------------------------------------------------------
#
# rulesync outputs — .claude/rules, .claude/skills, commands, subagents — are
# generated by postinstall and gitignored, so a clone carries none of them and
# `--ignore-scripts` above means the install did not produce them either. Only
# .claude/settings.json is committed.
#
# Without this step a session has the plugin-delivered skills (registered above)
# but not the repo's own two: animation-test-migration and technology-stack. It
# has to happen here rather than in a session, for the same reason marketplaces
# do — Claude Code enumerates skills at launch.
#
# Cheap: 1.7s locally for the claudecode target. The ag-dev-prompts fetch inside
# it will fail (no credentials at setup time) and rulesync then generates from the
# tracked .rulesync sources alone, which is the 14 local rules and 2 local skills.
# The session's own finish-setup.sh install regenerates the full set.
...
generate_claude_config() {
    ...
    slice="$(remaining)"
    ((slice > 10)) || {
        log_warn "no time left to generate .claude config"
        return 1
    }
    ((slice > 90)) && slice=90

    # Via the package script, not the shell script directly: the repo declares its
    # own invocation there (including AG_DEV_PROMPTS_REF), and duplicating it here
    # would render a settings.json that differs from the committed one and leave
    # every session with a dirty working tree.
    if node -e 'process.exit(require("./package.json").scripts["postinstall:setup-prompts"] ? 0 : 1)' 2>/dev/null; then
        with_timeout "$slice" yarn run postinstall:setup-prompts >/dev/null 2>&1 || return 1
    else
        local script="$REPO_ROOT/external/ag-shared/scripts/setup-prompts/setup-prompts.sh"
        [[ -x "$script" ]] || return 1
        with_timeout "$slice" "$script" --targets=claudecode >/dev/null 2>&1 || return 1
    fi

    local rules skills
    rules=$(find "$REPO_ROOT/.claude/rules" -name '*.md' 2>/dev/null | wc -l | tr -d ' ')
    skills=$(find "$REPO_ROOT/.claude/skills" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
    log_info "generated .claude config: ${rules} rules, ${skills} repo-local skills"
    [[ "$rules" -gt 0 || "$skills" -gt 0 ]]
}
...
main() {
    ...
    if step "yarn install" install_dependencies; then
        # Before the cache seed, so a session that restores the cache and a session
        # that inherits this tree both find the same generated config.
        step "generate .claude config" generate_claude_config || true
