...
export default {
    ...
    create(context) {
         ...
         */
        function requireModule(requirements, moduleId, reason, node) {
            if (!requirements.has(moduleId)) {
                requirements.set(moduleId, { reason, node });
            }
        }
         ...
         */
        function processSeriesObject(requirements, element, parentNode) {
            ...
            for (const prop of element.properties) {
                ...
                if (keyName === 'type') {
                    ...
                    if (seriesType) {
                        ...
                        if (moduleId) {
                            requireModule(requirements, moduleId, `series type '${seriesType}'`, prop);
                        }
                    }
                }
                ...
                if (keyName && seriesPluginToModule.has(keyName)) {
                    ...
                    requireModule(requirements, moduleId, `series option '${keyName}'`, prop);
                }
            }
        }
         ...
         */
        function processSeriesCallExpression(requirements, callExpr, parentNode) {
            ...
            if (callback && (callback.type === 'ArrowFunctionExpression' || callback.type === 'FunctionExpression')) {
                ...
                if (returnValue && returnValue.type === 'ObjectExpression') {
                    processSeriesObject(requirements, returnValue, parentNode);
                }
            }
        }
         ...
         */
        function processSeriesArray(requirements, seriesArray, parentNode) {
                ...
                for (const element of seriesArray.elements) {
                    ...
                    if (element.type === 'SpreadElement') {
                        ...
                        if (spreadArg.type === 'CallExpression') {
                            processSeriesCallExpression(requirements, spreadArg, parentNode);
                        }
                        continue;
                    }

                    processSeriesObject(requirements, element, parentNode);
                }
            } else if (seriesArray.type === 'CallExpression') {
                // Handle computed series like: series: data.map(d => ({ type: 'bar', ... }))
                processSeriesCallExpression(requirements, seriesArray, parentNode);
            }
        }
         ...
         */
        function processAxes(requirements, axesNode, parentNode) {
            ...
            for (const prop of axesNode.properties) {
                ...
                processAxisObject(requirements, prop.value, prop);
            }
        }
         ...
         */
        function processAxisObject(requirements, axisObj, parentNode) {
            for (const prop of axisObj.properties) {
                ...
                if (keyName === 'type') {
                    ...
                    if (axisType) {
                        ...
                        if (moduleId) {
                            requireModule(requirements, moduleId, `axis type '${axisType}'`, prop);
                        }
                    }
                }
                ...
                if (keyName && axisPluginToModule.has(keyName)) {
                    ...
                    requireModule(requirements, moduleId, `axis option '${keyName}'`, prop);
                }
            }
        }
         ...
         */
        function processPluginOption(requirements, keyName, valueNode, propNode) {
            if (pluginOptionToModule.has(keyName)) {
                ...
                requireModule(requirements, moduleId, `option '${keyName}'`, propNode);
            }
            ...
            if (keyName === 'annotations' && valueNode.type === 'ObjectExpression') {
                for (const nestedProp of valueNode.properties) {
                    ...
                    if (nestedKey && annotationsPluginToModule.has(nestedKey)) {
                        ...
                        requireModule(requirements, nestedModuleId, `annotations.${nestedKey} option`, nestedProp);
                    }
                }
            }
        }
         ...
         */
        function applyDefaultAxes(requirements) {
            for (const seriesType of seriesTypes) {
                ...
                if (defaults) {
                    ...
                    if (defaults.x && !explicitAxes.has('x')) {
                        ...
                        if (moduleId) {
                            requireModule(requirements, moduleId, `default axis for '${seriesType}' series`, null);
                        }
                    }
        ...
        return {
            ...
            Property(node) {
                const requirements = requiredModules;
                ...
                if (keyName === 'series') {
                    ...
                    processSeriesArray(requirements, node.value, node);
                } else if (keyName === 'axes') {
                    processAxes(requirements, node.value, node);
                    ...
                    if (node.value.type === 'ObjectExpression') {
                        processAxisObject(requirements, node.value, node);
                    }
                } else if (pluginOptionToModule.has(keyName)) {
                    processPluginOption(requirements, keyName, node.value, node);
                    ...
                    if (seriesTypeToModule.has(typeValue)) {
                        ...
                        requireModule(requirements, moduleId, `series type '${typeValue}'`, node);
                    }

                    // Check if it's an axis type
                    if (axisTypeToModule.has(typeValue)) {
                        const moduleId = axisTypeToModule.get(typeValue);
                        requireModule(requirements, moduleId, `axis type '${typeValue}'`, node);
                    }
                }
            },
            ...
            Literal(node) {
                ...
                if (seriesTypeToModule.has(value)) {
                    ...
                    requireModule(requirements, moduleId, `series type '${value}'`, node);
                }
                ...
                if (axisTypeToModule.has(value)) {
                    ...
                    requireModule(requirements, moduleId, `axis type '${value}'`, node);
                }
            },
            ...
            'Program:exit'() {
                ...
                applyDefaultAxes(requiredModules);
