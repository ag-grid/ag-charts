...
type VerifyAgBaseChartThemeOverrides<TDatum, TOptions = AgBaseChartOptions<TDatum>> = {
    [K in NonNullable<AgCartesianSeriesOptions<TDatum>['type']>]?: TOptions;
} & {
    [K in NonNullable<AgPolarSeriesOptions<TDatum>['type']>]?: TOptions;
} & {
    [K in NonNullable<AgHierarchySeriesOptions<TDatum>['type']>]?: TOptions;
} & {
    [K in NonNullable<AgSankeySeriesOptions<TDatum>['type']>]?: TOptions;
} & {
    common?: Partial<TOptions>;
};
...
let __VERIFY_THEME_OVERRIDES: Required<VerifyAgBaseChartThemeOverrides<never>> = undefined as any;
