...
describe('multi-series benchmark', () => {
    ...
    describe('after load', () => {
        ...
        benchmark('1x legend toggle', ctx, { expectedRelativeMB: 1, expectedCanvasCount: 5 }, async () => {
            await ctx.legendToggle();
            await ctx.legendToggle();
        });

        benchmark('10x legend toggle', ctx, { expectedRelativeMB: 1, expectedCanvasCount: 5 }, async () => {
            for (let i = 0; i < 5; i++) {
        ...
        benchmark('1x datum highlight', ctx, { expectedRelativeMB: 1, expectedCanvasCount: 5 }, async () => {
            const point = ctx.nodePositions[0][2];
            await ctx.hover(point.x, point.y);
        });

        benchmark('15x datum highlight', ctx, { expectedRelativeMB: 1, expectedCanvasCount: 5 }, async () => {
