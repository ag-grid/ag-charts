...
describe('simple-chart benchmark', () => {
    ...
    benchmark('initial load', ctx, { expectedRetainedSizeMB: 10, expectedCanvasCount: 4 }, async () => {
        await ctx.create();
    });
    ...
    describe('after load', () => {
        ...
        benchmark('10x legend toggle', ctx, { expectedRelativeMB: 5.5, expectedCanvasCount: 3 }, async () => {
            for (let i = 0; i < 5; i++) {
