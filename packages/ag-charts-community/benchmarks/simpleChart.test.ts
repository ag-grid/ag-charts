...
describe('simple-chart benchmark', () => {
    ...
    describe('after load', () => {
        ...
        benchmark('1x legend toggle', ctx, { expectedRelativeMB: 3.8, expectedCanvasCount: 4 }, async () => {
            await ctx.legendToggle();
            await ctx.legendToggle();
        });

        benchmark('10x legend toggle', ctx, { expectedRelativeMB: 1.9, expectedCanvasCount: 4 }, async () => {
            for (let i = 0; i < 5; i++) {
        ...
        benchmark('1x datum highlight', ctx, { expectedRelativeMB: 1.9, expectedCanvasCount: 4 }, async () => {
            const point = ctx.nodePositions[0][1];
            await ctx.hover(point.x, point.y);
        });

        benchmark('15x datum highlight', ctx, { expectedRelativeMB: 1.9, expectedCanvasCount: 4 }, async () => {
