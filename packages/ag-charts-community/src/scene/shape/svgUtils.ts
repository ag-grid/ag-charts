...
export function setSvgFontAttributes(element: SVGElement, options: FontOptions) {
    ...
    if (fontSize) element.setAttribute('font-size', String(fontSize));
...
export function setSvgStrokeAttributes(element: SVGElement, options: StrokeOptions) {
    ...
    if (strokeWidth) element.setAttribute('stroke-width', String(strokeWidth));
    if (strokeOpacity) element.setAttribute('stroke-opacity', String(strokeOpacity));
}
...
export function setSvgLineDashAttributes(element: SVGElement, options: LineDashOptions) {
    ...
    if (lineDash?.some((d) => d !== 0)) {
        ...
        if (lineDashOffset) element.setAttribute('stroke-dashoffset', String(lineDashOffset));
    }
}
