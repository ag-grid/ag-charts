...
export class BandScale extends BaseScale<DomainType, number> {
    ...
    constructor(domain: DomainType[], range: number[], options?: BandScaleOptions) {
        ...
        let start =
            domain.length === 1
                ? Math.min(r0, r1) + Math.abs(r1 - r0) * this.paddingOuter
                : Math.min(r0, r1) + (Math.abs(r1 - r0) - step * (domain.length - this.paddingInner)) * this.align;
    ...
    step() {
        ...
        if (count === 1) {
            const step = Math.abs(r1 - r0) * (1 - paddingOuter * 2);
            return this.round ? Math.round(step) : step;
        }
