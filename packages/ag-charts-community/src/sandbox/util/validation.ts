...
export function validateDefs<T extends object>(options: T, optionsDefs: OptionsDefs<T>, path = '') {
    ...
    function invalidMessage(key: string, value: unknown, validatorOrDefs: { [descriptionSymbol]?: string }) {
        let description = validatorOrDefs[descriptionSymbol];
        description = description ? `; expecting ${description}` : '';
        return `Option ${extendPath(key)} cannot be set to [${stringifyValue(value)}]${description}, ignoring.`;
    }
