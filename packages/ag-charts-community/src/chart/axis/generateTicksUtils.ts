...
export function getTimeIntervalTicks<S extends Scale<D, number, TickInterval<S>>, D>(
    ...
    const lastDomain = scale.domain.at(-1)! as unknown as Date;
    const dv0 = Math.min((scale.domain[0] as unknown as Date).valueOf(), lastDomain.valueOf());
    const dv1 = Math.max((scale.domain[0] as unknown as Date).valueOf(), lastDomain.valueOf());
