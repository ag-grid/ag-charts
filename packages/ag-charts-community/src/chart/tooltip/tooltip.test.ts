...
`);
    ...
    describe('AG-17064 Position Offset', () => {
        const testHover = async (x: number, y: number) => {
            await hoverAction(x, y)(chart);
            await compare();
        };
        ...
        it('should apply offset with placement right', async () => {
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'right', offset: 20 } },
            });
            await testHover(400, 200);
        });

        it('should apply offset with placement top', async () => {
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'top', offset: 20 } },
            });
            await testHover(400, 200);
        });

        it('should apply offset with diagonal placement top-right', async () => {
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'top-right', offset: 20 } },
            });
            await testHover(400, 200);
        });

        it('should remove gap when offset is 0', async () => {
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'top', offset: 0 } },
            });
            await testHover(400, 200);
        });

        it('should compose offset with xOffset and yOffset', async () => {
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'right', offset: 10, xOffset: 5, yOffset: -5 } },
            });
            await testHover(400, 200);
        });

        it('should adapt offset to fallback placement', async () => {
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: ['left', 'right'], offset: 15 } },
            });
            // Hover leftmost bar — tooltip should fall back to 'right'
            await testHover(160, 300);
            // Hover rightmost bar — tooltip should use 'left'
            await testHover(640, 300);
        });

        it('should show arrow with offset but hide it with xOffset', async () => {
            // offset alone — arrow visible
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'top', offset: 20 } },
            ...
            // xOffset added — arrow hidden
            chart = await createChart({
                data: barChartData,
                series: [{ type: 'bar', xKey: 'x', yKey: 'y' }],
                tooltip: { position: { anchorTo: 'node', placement: 'top', offset: 20, xOffset: 10 } },
            });
            await testHover(400, 200);
        });
    });
