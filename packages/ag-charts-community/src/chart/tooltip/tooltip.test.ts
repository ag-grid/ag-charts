...
describe('Tooltip', () => {
    ...
    describe('Realtime', () => {
        ...
        it('should update tooltip correctly', async () => {
            ...
            const element = getDocument('body').getElementsByClassName('ag-chart-tooltip').item(0) as HTMLDivElement;
            expect(element.textContent).toEqual('4: 1.5');

            await nextValue(10, 1.3249187570726666);
            expect(element.textContent).toEqual('5: 1.4');

            await nextValue(11, 1.2651169069335022);
            expect(element.textContent).toEqual('6: 1.2');

            await nextValue(12, 1.3627720015958902);
            expect(element.textContent).toEqual('7: 1.1');

            await nextValue(13, 1.490244608234256);
            expect(element.textContent).toEqual('8: 1.5');

            await nextValue(14, 1.490244608234256);
            expect(element.textContent).toEqual('9: 1.2');
        });
    });
});
