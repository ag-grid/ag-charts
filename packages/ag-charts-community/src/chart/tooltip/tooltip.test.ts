...
`);
    ...
    describe('Realtime', () => {
        it('should update tooltip correctly', async () => {
            ...
            expect(element.map((e) => e.textContent).join('')).toEqual('4 voltage 1.48');

            await nextValue(10, 1.3249187570726666);
            expect(element.map((e) => e.textContent).join('')).toEqual('5 voltage 1.4');

            await nextValue(11, 1.2651169069335022);
            expect(element.map((e) => e.textContent).join('')).toEqual('6 voltage 1.22');

            await nextValue(12, 1.3627720015958902);
            expect(element.map((e) => e.textContent).join('')).toEqual('7 voltage 1.11');

            await nextValue(13, 1.490244608234256);
            expect(element.map((e) => e.textContent).join('')).toEqual('8 voltage 1.49');

            await nextValue(14, 1.490244608234256);
            expect(element.map((e) => e.textContent).join('')).toEqual('9 voltage 1.17');
        });
    });
