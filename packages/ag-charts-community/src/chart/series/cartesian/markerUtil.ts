...
export function updateMarkerDatumStyles<D extends SeriesNodeDatum<number> & { style?: AgSeriesMarkerStyle }>(
    series: any,// TODO add typing
    opts: {
        datumSelection: Selection<Marker, D>;
        isHighlight: boolean;
    }
) {
    const { datumSelection, isHighlight } = opts;
    const { marker } = series.properties;

    const highlightedDatum = series.ctx.highlightManager.getActiveHighlight();
    datumSelection.each((node, datum) => {
        if (!datumSelection.isGarbage(node)) {
            ...
            const stylerStyle = series.getStyle(isHighlight, highlightState);
