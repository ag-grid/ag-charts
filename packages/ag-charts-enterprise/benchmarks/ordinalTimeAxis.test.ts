...
describe('ordinal time axis benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('axes-1M-ordinal-time', { isEnterprise: true });

    benchmark(
        'initial load',
        ctx,
        { expectedRetainedSizeMB: 165, expectedCanvasCount: 3, autoSnapshot: false },
        async () => {
            await ctx.create();
        }
    );
    ...
    describe('after load', () => {
        ...
        benchmark(
            '20x zoom',
            ctx,
            { expectedRelativeMB: 1, expectedCanvasCount: 3, autoSnapshot: false },
            async () => {
                for (let i = 0; i < 20; i++) {
