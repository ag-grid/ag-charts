...
describe('bar series benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('high-perf-bar', { isEnterprise: true });

    benchmark(
        'initial load',
        ctx,
        { expectedRetainedSizeMB: 207, expectedCanvasCount: 3, autoSnapshot: false },
        async () => {
            await ctx.create();
        },
        30_000
