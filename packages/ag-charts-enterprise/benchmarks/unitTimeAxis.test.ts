...
describe('unit time axis benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('axes-1M-unit-time', { isEnterprise: true });

    benchmark(
        'initial load',
        ctx,
        { expectedRetainedSizeMB: 232, expectedCanvasCount: 3, autoSnapshot: false },
        async () => {
            await ctx.create();
        }
    );
