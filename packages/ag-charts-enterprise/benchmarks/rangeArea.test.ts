...
describe('range area benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('high-perf-range-area', { isEnterprise: true });

    benchmark(
        'initial load',
        ctx,
        { expectedRetainedSizeMB: 45, expectedCanvasCount: 3, autoSnapshot: false },
        async () => {
            await ctx.create();
        },
        30_000
    ...
    describe('after load', () => {
        ...
        benchmark(
            '20x zoom',
            ctx,
            { expectedRelativeMB: 8, expectedCanvasCount: 3, autoSnapshot: false },
            async () => {
                for (let i = 0; i < 20; i++) {
