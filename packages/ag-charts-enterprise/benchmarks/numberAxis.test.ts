...
describe('number axis benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('axes-1M-number', { isEnterprise: true });

    benchmark(
        'initial load',
        ctx,
        { expectedRetainedSizeMB: 103, expectedCanvasCount: 3, autoSnapshot: false },
        async () => {
            await ctx.create();
        }
    );
