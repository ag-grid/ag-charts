...
describe('ohlc series benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('high-perf-ohlc', { isEnterprise: true });

    benchmark(
        'initial load',
        ctx,
        { expectedRetainedSizeMB: 255, expectedCanvasCount: 3, autoSnapshot: false },
        async () => {
            await ctx.create();
        },
        30_000
