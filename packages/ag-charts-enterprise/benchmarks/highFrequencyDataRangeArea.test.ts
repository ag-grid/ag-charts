...
describeWhenSupported('high-frequency data range-area benchmark', () => {
    const ctx = setupBenchmark<AgCartesianChartOptions>('high-freq-range-area', { isEnterprise: true });
    ...
    const rangeAreaGenerator = new HighFrequencyRangeAreaGenerator();

    let data: Datum[] = [];

    function createSeedData(count: number): Datum[] {
        rangeAreaGenerator.reset();
        return rangeAreaGenerator.take(count);
    }
