...
import {
    type ScaledBoxPlotValues,
    applyPositionOffset,
    calculateBoxPlotDimensions,
    validateScaledValues,
} from './boxPlotRenderUtils';
import { BoxPlotSeriesProperties } from './boxPlotSeriesProperties';
import type { BoxPlotNodeDatum } from './boxPlotTypes';
import { type BoxPlotStatisticalData, validateBoxPlotData } from './statisticalUtils';
...
> extends _ModuleSupport.SeriesNodeEvent<BoxPlotNodeDatum, TEvent> {
    ...
    constructor(type: TEvent, nativeEvent: Event, datum: BoxPlotNodeDatum, series: BoxPlotSeriesHybrid) {
...
export class BoxPlotSeriesHybrid extends _ModuleSupport.AbstractBarSeries<
    BoxPlotNode,
...
> {
    static readonly className = 'BoxPlotSeriesHybrid';
    ...
    /**
     * Validates box plot statistical data using utility functions
     */
    private validateStatisticalData(
        minValue: number,
        q1Value: number,
        medianValue: number,
        q3Value: number,
        ...
        const statisticalData: BoxPlotStatisticalData = {
            min: minValue,
            q1: q1Value,
            median: medianValue,
            q3: q3Value,
        ...
        const validation = validateBoxPlotData(statisticalData);
    ...
    override createNodeData() {
        ...
        rawData.forEach((datum, datumIndex) => {
            ...
            if (!this.validateStatisticalData(minValue, q1Value, medianValue, q3Value, maxValue)) {
                return;
            }

            const scaledValues: ScaledBoxPlotValues = {
            ...
            if (!validateScaledValues(scaledValues)) {
                return;
            }

            // Apply position offset using utility function
            const groupScaleOffset = groupScale.convert(String(groupIndex));
            scaledValues.xValue = applyPositionOffset(scaledValues.xValue, groupScaleOffset, barOffset, barWidth);

            // Use utility function for calculating dimensions
            const dimensions = calculateBoxPlotDimensions(scaledValues, barWidth, isVertical);
            ...
            nodeData.push({
                ...
                datumIndex,
                xKey,
                bandwidth: dimensions.bandwidth,
                scaledValues,
                midPoint: dimensions.midPoint,
                focusRect: dimensions.focusRect,
            });
        });
