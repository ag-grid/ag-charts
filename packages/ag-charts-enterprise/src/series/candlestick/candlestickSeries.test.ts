...
describe('CandlestickSeries', () => {
    ...
    // Candlestick/OHLC implement no animation hooks, so every action must land completely on the
    ...
    describe('animation -test page actions', () => {
        ...
        // The y-axis is pinned so candle heights reflect the data alone; the x-axis is left unpinned
        // so add/remove exercise the real band reflow.
        const candleData = () => [
        ...
        // Anti-vacuity: three subpaths over a real pixel height means the candle is drawn at full
        // size, not parked collapsed at a baseline waiting to grow.
        ...
        it('update value: the changed candle snaps to its new range', async () => {
            ...
            // Anti-vacuity: constancy would pass vacuously if nothing moved.
        ...
        it('add data: a new candle appears instantly and the layout snaps', async () => {
            ...
            // The y-axis is pinned, so a survivor rescale could only surface as a height drift here.
            expect(
