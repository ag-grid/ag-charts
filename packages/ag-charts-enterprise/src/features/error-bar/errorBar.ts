...
export class ErrorBars extends _ModuleSupport.BaseModuleInstance implements _ModuleSupport.SeriesOptionInstance {
    ...
    private getUnstackPropertyDefinition(opts: PropertyDefinitionOpts) {
        ...
        if (yLowerKey != null && yUpperKey != null) {
            props.push(
                valueProperty(cartesianSeries, yLowerKey, isContinuousY, { id: `${yErrorsID}-lower` }),
                valueProperty(cartesianSeries, yUpperKey, isContinuousY, { id: `${yErrorsID}-upper` })
            );
        }
        if (xLowerKey != null && xUpperKey != null) {
            props.push(
                valueProperty(cartesianSeries, xLowerKey, isContinuousX, { id: `${xErrorsID}-lower` }),
                valueProperty(cartesianSeries, xUpperKey, isContinuousX, { id: `${xErrorsID}-upper` })
            );
        }
        return props;
    }
