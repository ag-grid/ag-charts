...
> {
    ...
    constructor(ctx: TextualStartEndStateMachineContext<Datum, Node>) {
        ...
        const actionExitAndClear = () => {
            ctx.delete();
        };

        const actionUpdateEnd = ({ point }: { point: Point }) => {
        ...
        const actionSaveEnd = ({ point }: { point: Point }) => {
            ctx.datum()?.set({ end: point });
        ...
        const guardExitAndClear = ({ key }: { key: string }) => key === 'Escape';
        const guardExitAndSave = ({ key, shiftKey }: { key: string; shiftKey: boolean }) =>
            !shiftKey && key === 'Enter';
        ...
        super('start', {
            ...
            'waiting-first-render': {
                ...
                hover: actionUpdateEnd,
                click: {
                    target: 'edit',
                    action: actionSaveEnd,
                },
                cancel: StateMachine.parent,
            },
            edit: {
                ...
                keyDown: [
                    {
                        guard: guardExitAndClear,
                        target: StateMachine.parent,
                        action: actionExitAndClear,
                    },
                    {
                        guard: guardExitAndSave,
                        target: StateMachine.parent,
                        action: actionSave,
                    },
                ],
