...
export abstract class AgChartsBase<Options extends {}> implements AfterViewInit, OnChanges, OnDestroy {
    ...
    private patchChartOptions(propsOptions: any): any {
        // Every deref below assumes an options object, so an invalid `[options]` input would either throw a raw
        // TypeError or reach the chart as a `{ container }` object its own check cannot reject. Validate first
        // and build from what comes back, so the chart reports the diagnostic.
