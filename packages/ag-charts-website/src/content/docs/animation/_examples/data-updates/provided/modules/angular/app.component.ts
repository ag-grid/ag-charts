...
import type { AgCartesianChartOptions, AgPolarChartOptions } from 'ag-charts-enterprise';
...
@Component({
    ...
    template: `<div style="display: flex; flex-direction: column">
        <div
            style="flex: none; display: flex; flex-direction: row; justify-content: center; align-items: center; gap: 0.5em"
        >
            <div style="display: flex; flex-direction: row; justify-content: center; gap: 0.5em">
                <button (click)="changeSeriesBar()">Bar</button>
                <button (click)="changeSeriesLine()">Line</button>
                <button (click)="changeSeriesArea()">Area</button>
                <button (click)="changeSeriesDonut()">Pie &amp; Donut</button>
            </div>
        </div>
        <div
            style="flex: none; display: flex; flex-direction: row; justify-content: center; align-items: center; gap: 0.5em"
        >
            <div style="display: flex; flex-direction: row; justify-content: center; gap: 0.5em">
                <button class="animation-data-updates__action" (click)="add()" [disabled]="tickingUpdates">Add</button>
                <button class="animation-data-updates__action" (click)="remove()" [disabled]="tickingUpdates">
                    Remove
                </button>
                <button class="animation-data-updates__action" (click)="update()" [disabled]="tickingUpdates">
                    Update
                </button>
            </div>
            <button class="animation-data-updates__action" (click)="addRemoveUpdate()">
                Add &amp; Remove &amp; Update
            </button>
            <button class="animation-data-updates__toggle-ticking" (click)="toggleTickingUpdates()">
...
export class AppComponent {
    public options: AgCartesianChartOptions | AgPolarChartOptions;
    ...
    toggleTickingUpdates = () => {
        ...
        if (this.tickingUpdates) {
            this.interval = setInterval(this.updateTick, 2000);
