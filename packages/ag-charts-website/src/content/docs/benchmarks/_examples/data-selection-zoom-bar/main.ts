...
const SERIES_TYPES = ['bar', 'stacked-bar', 'range-bar'] as const;
...
function buildSeries(type: SeriesType): AgCartesianChartOptions['series'] {
    switch (type) {
        ...
        case 'stacked-bar':
            return [
                { type: 'bar', xKey: 'timestamp', yKey: 'open', stacked: true, selection: { enabled: true } },
                { type: 'bar', xKey: 'timestamp', yKey: 'close', stacked: true, selection: { enabled: true } },
            ];
        case 'range-bar':
            return [
                {
