...
function changeLayerSpacing(event: any) {
    const value = Number(event.target.value);
    document.getElementById('layerSpacingValue')!.innerHTML = String(value);
    (options.series![0] as AgOrganizationSeriesOptions).layerSpacing = value;
    chart.update(options);
}
