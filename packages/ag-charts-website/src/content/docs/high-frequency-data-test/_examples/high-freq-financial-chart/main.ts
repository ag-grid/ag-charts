...
function createLiveDatumFactory(getData: () => any[], mode = 'append') {
    return () => {
        ...
        if (mode === 'append') {
            ...
            date = lastDatum ? new Date(lastDatum.date.getTime() + DATA_INTERVAL_MS) : new Date(START_DATE);
            index = Math.floor((date.getTime() - START_DATE.getTime()) / DATA_INTERVAL_MS);
        } else {
            const firstDatum = currentData.at(0);
            date = firstDatum ? new Date(firstDatum.date.getTime() - DATA_INTERVAL_MS) : new Date(START_DATE);
            index = Math.floor((date.getTime() - START_DATE.getTime()) / DATA_INTERVAL_MS);
        }
...
function createSeedData(count = 30 * 60 * (1000 / DATA_INTERVAL_MS)) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        date = new Date(date.getTime() + DATA_INTERVAL_MS);
    }
...
const options: AgFinancialChartOptions = {
    ...
    data: initialData,
    title: { text: 'High Frequency Financial Chart' },
    dateKey: 'date',
    openKey: 'open',
    highKey: 'high',
    lowKey: 'low',
