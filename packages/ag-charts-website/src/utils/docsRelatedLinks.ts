 ...
 * the page's own nav group. Derived rather than curated so the pages cannot drift out of date.
 */

/**
 * The nav fields these helpers read. Declared structurally rather than imported: the collection
 * types come from `astro:content`, which cannot resolve under vitest, and `MenuItem` describes the
 * site-header menu, whose children are `items`.
 */
...
/** A docs page name (`bar-series`), titled from the nav, or an explicit title and URL. */
...
function toUrl(item: DocsNavItem, framework: Framework, siteRoot?: string): string {
...
function findSiblings(items: DocsNavItem[], pageName: string, framework: Framework): DocsNavItem[] | undefined {
...
function findTitle(items: DocsNavItem[], pageName: string): string | undefined {
    for (const item of items) {
