...
export const getChartScriptPath = (sitePrefix?: string) => {
    ...
    return pathJoin({ path: [sitePrefixUrl, '/dev/ag-charts-community/dist/main.umd.js'] });
};

export const getChartEnterpriseScriptPath = (sitePrefix?: string) => {
    const sitePrefixUrl = sitePrefix ? sitePrefix : '';
    return pathJoin({ path: [sitePrefixUrl, '/dev/ag-charts-enterprise/dist/main.umd.js'] });
};
