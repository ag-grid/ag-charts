...
import { DISABLE_MARKDOWN_DOCS, LIVE_SITEMAP_URL, PRODUCTION_CHARTS_SITE_URL, agChartsVersion } from '@constants';
...
// Served at /charts/llms.txt. Generated on every build from the canonical base URL and the
// current major version, so it cannot drift from the shipped product.
...
export async function GET() {
    ...
    // Only the docs nav: the API menu is an eight-entry top bar whose pages the docs nav already
    // lists, and the generated options/themes reference is reached through the sitemap index.
    const docsIndex = navSectionsToIndex({
        sections: docsNavData.sections,
    ...
    const siteIndex = buildSitePageIndex({
        ...
        navPages: navPageNames(docsNavData.sections),
        canonicalFramework: CANONICAL_FRAMEWORK,
    });

    const output = buildLlmsTxt({
        siteRoot,
        majorVersion: parseVersion(agChartsVersion).major,
        chartsDocsPrefix: getFrameworkPath(CANONICAL_FRAMEWORK),
