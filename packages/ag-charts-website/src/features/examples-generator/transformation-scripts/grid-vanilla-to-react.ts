...
function getImports(
  ...
  return (importType === "packages" ? getPackageImports : getModuleImports)(
    bindings,
    componentFileNames,
    allStylesheets
  );
...
export function vanillaToReact(
  bindings: any,
  componentFilenames: string[],
  allStylesheets: string[]
  ...
  const eventHandlers = bindings.eventHandlers.map((event) =>
    convertFunctionToProperty(event.handler)
  );
  const externalEventHandlers = bindings.externalEventHandlers.map((handler) =>
    convertFunctionToProperty(handler.body)
  );
  const instanceMethods = bindings.instanceMethods.map(
    convertFunctionToProperty
  );
  const componentEventAttributes = bindings.eventHandlers.map(
    (event) => `${event.handlerName}={this.${event.handlerName}.bind(this)}`
  );
  ...
  if (data) {
    ...
    if (data.callback.indexOf("api.setRowData") >= 0) {
      setRowDataBlock = data.callback.replace(
  ...
  return (importType) => {
    ...
    componentAttributes.push.apply(
      componentAttributes,
      componentEventAttributes
    );

    if (data && data.callback.indexOf("api.setRowData") >= 0) {
      if (
        stateProperties.filter((item) => item.indexOf("rowData") >= 0)
          .length === 0
      ) {
      ...
      if (
        componentAttributes.filter((item) => item.indexOf("rowData") >= 0)
          .length === 0
      ) {
