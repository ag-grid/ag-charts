...
describe("convertTemplate", () => {
  it("converts event handlers", () => {
    ...
    expect(converted).toBe(
      "<button onClick={() => this.foo(true)}>Hello!</button>"
    );
  });

  it("removes trailing event parameter from handler calls", () => {
    const converted = convertTemplate(
      '<button onchange="foo(true, event)">Hello!</button>'
    );

    expect(converted).toBe(
      "<button onChange={() => this.foo(true)}>Hello!</button>"
    );
  });

  it("removes non-first event parameter from handler calls", () => {
    const converted = convertTemplate(
      '<button oninput="foo(true, event, false)">Hello!</button>'
    );

    expect(converted).toBe(
      "<button onInput={() => this.foo(true, false)}>Hello!</button>"
    );
  });

  it("leaves first event parameter from handler calls", () => {
    const converted = convertTemplate(
      '<button onclick="foo(event)">Hello!</button>'
    );

    expect(converted).toBe(
      "<button onClick={() => this.foo(event)}>Hello!</button>"
    );
  });

  it("handles multiple event handlers", () => {
    const template =
      '<div onclick="foo(true)" ondrop="bar(false)">Hello!</div>';
    const converted = convertTemplate(template);

    expect(converted).toBe(
      "<div onClick={() => this.foo(true)} onDrop={() => this.bar(false)}>Hello!</div>"
    );
  });
  ...
  it("fixes casing for dragover", () => {
    ...
    expect(converted).toBe(
      "<div onDragOver={() => this.foo(true)}>Hello!</div>"
    );
  });
  ...
  it("fixes casing for dragstart", () => {
    ...
    expect(converted).toBe(
      "<div onDragStart={() => this.foo(true)}>Hello!</div>"
    );
  });
  ...
  it("ensures input value attributes are renamed", () => {
    ...
    expect(converted).toBe(
      '<input type="text" defaultValue="foo" maxLength="20" />'
    );
  });
  ...
  it("converts styles", () => {
    const template =
      '<div style="background-color: #ff0000; height: 80%">Hello!</div>';
    const converted = convertTemplate(template);

    expect(converted).toBe(
      '<div style={{"backgroundColor":"#ff0000","height":"80%"}}>Hello!</div>'
    );
  });
});
