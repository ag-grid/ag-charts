...
export const toInput = (property: any) =>
  `[${property.name}]="${property.name}"`;
export const toConst = (property: any) =>
  `[${property.name}]="${property.value}"`;
export const toOutput = (event: any) =>
  `(${event.name})="${event.handlerName}($event)"`;
  ...
  if (property.typings) {
    ...
    if (!["number", "string", "boolean"].includes(typing)) {
      ...
      if (property.name === "columnDefs") {
        ...
        typeName = property.value.includes("children")
          ? "(ColDef | ColGroupDef)[]"
          : "ColDef[]";
      }
...
export const toAssignment = (property: any) =>
