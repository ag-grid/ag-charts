...
export function vanillaToReactFunctionalTs(
  ...
  return (importType) => {
    ...
    const eventHandlers = bindings.eventHandlers
      .map((event) =>
        convertFunctionToConstCallbackTs(event.handler, callbackDependencies)
      )
      .map(thisReferenceConverter)
      .map(gridInstanceConverter);
    const externalEventHandlers = bindings.externalEventHandlers
      .map((handler) =>
        convertFunctionToConstCallbackTs(handler.body, callbackDependencies)
      )
      .map(thisReferenceConverter)
      .map(gridInstanceConverter);
    const instanceMethods = bindings.instanceMethods
      .map((instance) =>
        convertFunctionToConstCallbackTs(instance, callbackDependencies)
      )
      .map(thisReferenceConverter)
      .map(gridInstanceConverter);
...
`;
    ...
    if (
      generatedOutput.includes(rowDataState) &&
