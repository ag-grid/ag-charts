...
export function getOnGridReadyCode(bindings: any): string {
  ...
  if (data) {
    ...
    const setRowDataBlock =
      callback.indexOf("api.setRowData") >= 0
        ? callback.replace(
...
export function isExternalVueFile(componentFileNames, component) {
  return (
    ...
    componentFileNames.some((fileName) =>
      fileName.toUpperCase().includes(component.toUpperCase())
    )
  );
}
...
export function getPropertyBindings(
  ...
  bindings.properties
    .filter(
      (property) =>
        property.name !== "onGridReady" && property.name !== "columnDefs"
    )
    .forEach((property) => {
      if (bindings.vuePropertyBindings[property.name]) {
        const parsedObj = JSON5.parse(
          bindings.vuePropertyBindings[property.name]
        );
        const panelArrayName = parsedObj["toolPanels"]
          ? "toolPanels"
          : "statusPanels";
          ...
          parsedObj[panelArrayName].forEach((panel) => {
            Object.keys(panel).forEach((panelProperty) => {
              if (
                ...
                const parsedValue = panel[panelProperty].replace(
                  "AG_LITERAL_",
                  ""
                );
        ...
        if (bindings.frameworkComponents) {
          const userAgComponents = OVERRIDABLE_AG_COMPONENTS.filter(
            (agComponentName) =>
              bindings.frameworkComponents.some(
                (component) => component.name === agComponentName
              )
          ).map(
            (agComponentName) => `${agComponentName}: '${agComponentName}'`
          );
  ...
  if (bindings.data && bindings.data.callback.indexOf("api.setRowData") >= 0) {
    if (
      propertyAttributes.filter((item) => item.indexOf(":rowData") >= 0)
        .length === 0
    ) {
    ...
    if (
      propertyVars.filter((item) => item.indexOf("rowData") >= 0).length === 0
    ) {
...
export function getTemplate(bindings: any, attributes: string[]): string {
  ...
  const style = gridSettings.noStyle
    ? ""
    : `style="width: ${gridSettings.width}; height: ${gridSettings.height};"`;
  ...
  const template = bindings.template
    ? bindings.template.replace(templatePlaceholder, agGridTag)
    : agGridTag;
...
export function addToVueComponents(
  componentFileNames,
  vueComponents,
  property,
  componentName
  ...
  if (
    ...
    isExternalVueFile(componentFileNames, componentName)
  ) {
...
export function convertColumnDefs(
  rawColumnDefs,
  userComponentNames,
  vueComponents,
  componentFileNames
  ...
  const processObject = (obj) => {
    ...
    return output
      ...
      .replace(/"AG_FUNCTION_(.*?)"/g, (match) =>
        parseFunction(JSON.parse(match))
      );
  };
      ...
      if (columnProperty === "children") {
        ...
        if (isParamsProperty(columnProperty)) {
          if (value.cellRenderer) {
            const component = `${value.cellRenderer.replace(
              "AG_LITERAL_",
              ""
            )}`;
          ...
          if (value.filters) {
            value.filters.forEach((filter) => {
              if (filter.floatingFilterComponent) {
                const component = `${filter.floatingFilterComponent.replace(
                  "AG_LITERAL_",
                  ""
                )}`;
                if (isExternalVueFile(componentFileNames, component)) {
        ...
        if (typeof value === "string") {
          if (value.startsWith("AG_LITERAL_")) {
            ...
            if (
              isComponent(columnProperty) &&
              isExternalVueFile(componentFileNames, parsedValue)
            ) {
            ...
            columnProperties.push(
              `${columnProperty}:"${value.replace(/(?<!\\)"/g, "'")}"`
            );
          }
...
export function convertDefaultColDef(
  defaultColDef,
  vueComponents,
  componentFileNames
): string {
  ...
  perLine.forEach((line) => {
    if (
      ...
      (line.includes("headerComponent") &&
        !line.includes("headerComponentParams")) ||
...
export const getColumnDefs = (
  ...
  const columnDefs = bindings.parsedColDefs
    ...
    : null;
  if (!columnDefs) {
    const columnDefProperty = bindings.properties.filter(
      (property) => property.name === "columnDefs"
    );
    if (
      columnDefProperty &&
      columnDefProperty.length === 1 &&
      columnDefProperty[0].value
    ) {
