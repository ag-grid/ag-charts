...
function internalParser(
    ...
    tsCollectors.push({
        ...
        apply: (bindings, node) =>
            bindings.instanceMethods.push(removeInScopeJsDoc(tsGenerateWithReplacedGridOptions(node, tsTree))),
    });
    ...
    PROPERTIES.forEach((propertyName) => {
        ...
        tsGridOptionsCollectors.push({
            ...
            apply: (bindings, node: ts.PropertyAssignment) => {
                if (
                    processColDefsForFunctionalReactOrVue(propertyName, exampleType, exampleSettings, providedExamples)
                ) {
                    const parent = node;
