...
function tsNodeIsSimpleFetchRequest(node) {
  if (ts.isCallExpression(node)) {
    node = node as any;
    const isFetch =
      ts.isCallExpression(node) && node.expression.getText() === "fetch";
    return isFetch;
  }
}
...
function processColDefsForFunctionalReactOrVue(
  propertyName: string,
  exampleType,
  exampleSettings,
  providedExamples
  ...
  if (propertyName === "columnDefs") {
    return (
      ...
      (exampleType === "mixed" &&
        !(
          providedExamples["reactFunctional"] &&
          providedExamples["vue"] &&
          providedExamples["vue3"]
        ))
    );
  }
...
function processComponentsForVue(
  propertyName: string,
  exampleType,
  providedExamples
) {
  if (propertyName === "components") {
    return (
      exampleType === "generated" ||
      (exampleType === "mixed" &&
        !(providedExamples["vue"] && providedExamples["vue3"]))
    );
  }
...
function processDefaultColumnDefForVue(
  propertyName: string,
  exampleType,
  providedExamples
) {
...
function processGlobalComponentsForVue(
  propertyName: string,
  exampleType,
  providedExamples
) {
...
export function parser(
  ...
  const bindings = internalParser(
    examplePath,
    { fileName, srcFile, includeTypes: false },
    html,
    exampleSettings,
    exampleType,
    providedExamples
  ...
  const typedBindings = internalParser(
    examplePath,
    { fileName, srcFile, includeTypes: true },
    html,
    exampleSettings,
    exampleType,
    providedExamples
...
function getTypeLookupFunc(includeTypes, fileName) {
  ...
  if (includeTypes) {
    ...
    const optionsFile = program
      .getSourceFiles()
      .find((f) => f.fileName.endsWith("gridOptions.d.ts"));
    if (optionsFile) {
      cachedProgram = program;
      const gridOptionsInterface = optionsFile.statements.find(
        (i: ts.Node) =>
          ts.isInterfaceDeclaration(i) && i.name.getText() == "GridOptions"
      ...
      lookupType = (propName: string) => {
        ...
        if (pop && pop.type) {
          return {
            typeName: pop.type.getText(),
            typesToInclude: getTypes(pop.type),
          };
        } else {
          console.warn(
            `Could not find GridOptions property ${propName} for example file ${fileName}`
          );
        }
        return undefined;
      };
...
function internalParser(
  ...
  const tsTree = includeTypes
    ? parseFile(srcFile)
    : parseFile(readAsJsFile(srcFile));
  ...
  tsCollectors.push({
    ...
    apply: (bindings, node) =>
      bindings.instanceMethods.push(
        removeInScopeJsDoc(tsGenerateWithReplacedGridOptions(node, tsTree))
      ),
  });
  ...
  tsCollectors.push({
    matches: (node) =>
      tsNodeIsUnusedFunction(node, registered, unboundInstanceMethods),
    apply: (bindings, node) => {
      const util = tsGenerate(node, tsTree).replace(
        /gridOptions/g,
        "gridInstance"
      );
  ...
  tsCollectors.push({
    ...
    apply: (bindings, node) =>
      bindings.utils.push(tsGenerate(node.parent, tsTree)),
  });
  ...
  tsOnReadyCollectors.push({
    ...
    apply: (bindings, node) => {
      ...
      const callback = tsGenerate(
        node.parent.parent.parent.parent.arguments[0].body,
        tsTree
      ).replace(/gridOptions/g, "params");
      bindings.data = { url, callback };
    },
  });
    ...
    matches: (node) => {
      if (
        ...
        if (node.expression.expression.name.getText() === "sizeColumnsToFit") {
          ...
          if (
            ts.isCallExpression(domContentLoaded) &&
            domContentLoaded.arguments.length > 0
          ) {
            return (
              domContentLoaded.arguments[0].getText() === "'DOMContentLoaded'"
            );
          }
        }
      }
  ...
  EVENTS.forEach((eventName) => {
    ...
    tsGridOptionsCollectors.push({
      ...
      matches: (node) =>
        tsNodeIsPropertyWithName(node, onEventName) &&
        onEventName !== "onGridReady",
      apply: (bindings, node: ts.PropertyAssignment) => {
        ...
        const eventHandler = tsGenerateWithReplacedGridOptions(
          node.initializer,
          tsTree
        );
    ...
    tsCollectors.push({
      ...
      apply: (bindings, node: ts.NamedDeclaration) => {
        ...
        bindings.properties.push({
          name: functionName,
          value: null,
          typings: gridOpsTypeLookup(functionName),
        });
      },
    });
  });
  ...
  const tsExtractColDefsStr = (node: any): string => {
    ...
    // for each column def
    for (
      let columnDefIndex = 0;
      columnDefIndex < node.elements.length;
      columnDefIndex++
    ) {
      ...
      for (
        ...
        const columnDefProperty: any =
          columnDef.properties[colDefPropertyIndex];
  ...
  const tsArrayStr = (node: any): string => {
    ...
    for (let index = 0; index < node.elements.length; index++) {
      ...
      if (!ts.isObjectLiteralExpression(item)) {
        ...
        for (
          let colDefPropertyIndex = 0;
          colDefPropertyIndex < item.properties.length;
          colDefPropertyIndex++
        ) {
  ...
  PROPERTIES.forEach((propertyName) => {
    ...
    tsCollectors.push({
      ...
      apply: (bindings, node) => {
        try {
          if (
            processColDefsForFunctionalReactOrVue(
              propertyName,
              exampleType,
              exampleSettings,
              providedExamples
            ...
            if (
              ...
              bindings.parsedColDefs = tsExtractAndParseColDefs(
                node.initializer
              );
            }
          }
    ...
    tsGridOptionsCollectors.push({
      ...
      apply: (bindings, node: ts.PropertyAssignment) => {
        ...
        if (
          processComponentsForVue(propertyName, exampleType, providedExamples)
        ) {
          ...
          if (ts.isObjectLiteralExpression(compsNode)) {
            for (const componentDefinition of compsNode.properties) {
              ...
              if (
                !ts.isCallExpression(comp.initializer) &&
                !ts.isFunctionExpression(comp.initializer)
              ) {
          ...
          if (ts.isObjectLiteralExpression(node.initializer)) {
            ...
            for (const property of node.initializer.properties) {
              if (ts.isArrayLiteralExpression((property as any).initializer)) {
                props.push(
                  `${property.name.getText()}:${tsArrayStr(
                    (property as any).initializer
                  )}`
                );
              } else {
                props.push(
  ...
  tsGridOptionsCollectors.push({
    ...
    apply: (bindings, node: ts.PropertyAssignment) => {
      if (node.initializer) {
        bindings.onGridReady = tsGenerate(
          (node.initializer as any).body,
          tsTree
        ).replace(/gridOptions/g, "params");
      } else {
  ...
  if (inlineClass) {
    const theme = inlineClass
      .split(" ")
      .filter((className) => className.indexOf("ag-theme") >= 0);
    exampleSettings.theme =
      theme && theme.length > 0 ? theme[0] : "ag-theme-alpine";
  }
