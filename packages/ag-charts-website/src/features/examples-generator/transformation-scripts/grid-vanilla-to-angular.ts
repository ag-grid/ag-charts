...
function getOnGridReadyCode(
  ...
  if (data) {
    ...
    if (callback.indexOf("api!.setRowData") !== -1) {
      const setRowDataBlock = callback.replace(
        "params.api!.setRowData(data)",
        "this.rowData = data"
      );
      additionalLines.push(
        `this.http.get<${rowDataType}[]>(${url}).subscribe(data => ${setRowDataBlock});`
      );
    } else {
      additionalLines.push(
        `this.http.get<${rowDataType}[]>(${url}).subscribe(data => ${callback});`
      );
    }
  }
  ...
  if (hasApi || hasColApi || additionalLines.length > 0) {
    return `
        onGridReady(params: GridReadyEvent${gridReadyEventParam}) {
            ${hasApi ? "this.gridApi = params.api;" : ""}${
      hasColApi ? "this.gridColumnApi = params.columnApi;" : ""
    }${
...
export function vanillaToAngular(
  ...
  const externalEventHandlers = bindings.externalEventHandlers.map((handler) =>
    removeFunctionKeyword(handler.body)
  );
  ...
  return (importType) => {
    ...
    const template = getTemplate(
      bindings,
      propertyAttributes.concat(eventAttributes)
    );
