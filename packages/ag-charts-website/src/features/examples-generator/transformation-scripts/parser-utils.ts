...
export function readAsJsFile(
  srcFile,
  options: { includeImports: boolean } = undefined
) {
...
export function modulesProcessor(modules: string[]) {
  ...
  requiredModules.forEach((requiredModule) => {
    moduleImports.push(
      `import { ${requiredModule.exported} } from '${requiredModule.module}';`
    );
...
export function isInstanceMethod(methods: string[], property: any): boolean {
  return (
    methods.map(getFunctionName).filter((name) => name === property.name)
      .length > 0
  );
}
...
export function tsNodeIsTopLevelVariable(
  node: ts.Node,
  registered: string[] = []
) {
...
export function tsNodeIsInScope(
  node: any,
  unboundInstanceMethods: string[]
): boolean {
  return (
...
export function tsNodeIsUnusedFunction(
  node: any,
  used: string[],
  unboundInstanceMethods: string[]
): boolean {
...
function isDeclareStatement(node) {
  return (
    node &&
    node.modifiers &&
    node.modifiers.some((m) => m.getText() === "declare")
  );
}
...
export const recognizedDomEvents = [
  "click",
  "change",
  "input",
  "dragover",
...
export function extractEventHandlers(domTree: any, eventNames: string[]) {
  ...
  return flatMap(eventNames, (event: string) =>
    getHandlerAttributes(event).map(extractEventHandlerBody)
  );
}
...
export function extractTypeInfoForVariable(
 ...
 */
export function convertImportPath(
  ...
  if (convertToPackage) {
    ...
    if (modulePackage.includes("@ag-grid-community/core/dist")) {
      return modulePackage.replace(
        "@ag-grid-community/core/dist",
        "ag-grid-community/dist"
      );
    }
...
export function getImport(filename: string) {
  ...
  const componentName =
    componentFileName[0].toUpperCase() + componentFileName.slice(1);
...
export function addBindingImports(
  ...
  const chartsEnterprise = bindingImports.some((i) =>
  ...
  Object.entries(workingImports).forEach(
    ([k, v]: [string, { namedImport: string; imports: string[] }]) => {
      let unique = [...new Set(v.imports)].sort();

      if (convertToPackage && k.includes("ag-grid")) {
        // Remove module related imports
        unique = unique.filter(
          (i) => !i.includes("Module") || i == "AgGridModule"
        );
      ...
      if (unique.length > 0 || v.namedImport) {
        ...
        imports.push(
          `import ${namedImport}${joiningComma}${importStr} from ${k};`
        );
      }
    }
  );
...
export function getModuleRegistration({
  ...
  moduleRegistration.push(
    `ModuleRegistry.registerModules(${gridSuppliedModules})`
  );
  return moduleRegistration;
}

export function handleRowGenericInterface(
  fileTxt: string,
  tData: string
): string {
  if (tData) {
...
export function addGenericInterfaceImport(
  imports: string[],
  tData: string,
  bindings
) {
