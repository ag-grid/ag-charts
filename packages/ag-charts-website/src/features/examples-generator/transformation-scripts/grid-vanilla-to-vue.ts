  ...
  if (componentFileNames) {
    imports.push(
      ...componentFileNames.map((componentFileName) =>
        getImport(componentFileName, "Vue", "")
      )
    );
  }
...
export function vanillaToVue(
  ...
  const vueComponents = bindings.components.map(
    (component) => `${component.name}:${component.value}`
  );
  ...
  const eventAttributes = bindings.eventHandlers
    ...
    .map(toOutput);
  const [eventHandlers, externalEventHandlers, instanceMethods, utilFunctions] =
