/**
 * On-page copy for the gallery hub and its example pages, written out in full. An example's title,
 * H1, meta description and intro are each edited directly; the get-started line every example page
 * closes with is the one piece they share.
 */
...
/**
 * The line every example page closes with, served as a paragraph of its own. It is boilerplate
 * rather than per-example copy: appended to each intro behind a hard line break, it read as a new
 * line with no gap before it.
 */
export const GALLERY_GET_STARTED_COPY =
...
export interface GalleryExampleCopy {
    ...
    description: string;
    /** Indexable body copy, rendered directly under the H1, above `GALLERY_GET_STARTED_COPY`. */
    intro: string;
}
...
export const GALLERY_EXAMPLE_COPY: Record<string, GalleryExampleCopy> = {
    ...
    'horizontal-range-bar-with-labels': {
        ...
        intro: 'This example shows a [horizontal range bar chart](/r/range-bar-series/#horizontal-range-bar) built with AG Charts, reading each bar as a change between two values rather than a static range. An [item styler](/r/stylers/#item-stylers) colours a bar by the direction of that change, a [label formatter](/r/range-bar-series/#labels) prints an arrow only where the swing is large enough to matter, and the value axis uses a fixed [step interval](/r/axes-intervals/#step) with a [cross line](/r/axes-cross-lines/#adding-cross-lines) marking the prior average.',
    },
    ...
    'heatmap-with-labels': {
        ...
        intro: 'This example shows a [heatmap chart](/r/heatmap-series/) built with AG Charts, printing a [formatted value](/r/formatters/#property-formatters) inside every cell so the grid can be read exactly as well as at a glance, which suits a heatmap small enough for the numbers to fit. [Cell labels](/r/heatmap-series/#labels) carry a formatter, the y-axis is [moved to the right](/r/axes-position/#axis-placement), and the [gradient legend](/r/colour-scale/#gradient-legend) labels are formatted to match the cells.',
    },
    ...
    sankey: {
        ...
        intro: 'This example shows a [sankey chart](/r/sankey-series/) built with AG Charts, tracing how a quantity divides and recombines as it passes between stages, visualised by the width of each link between the nodes. Nodes use [centre alignment](/r/sankey-series/#horizontal-alignment) and their labels sit [inside the node](/r/sankey-series/#label-placement).',
    },
    ...
    'chord-customisation': {
        ...
        intro: "This example shows a [chord chart](/r/chord-series/) built with AG Charts, weighting a dense set of connections so the strongest dominate: an [item styler](/r/stylers/#item-stylers) scales each ribbon's opacity by its volume, over and above the width it already carries. [Node width](/r/chord-series/#node-style) is widened with rounded corners and the [link stroke and opacity](/r/chord-series/#link-style) are styled.",
    },
    ...
    'simple-funnel': {
        ...
        intro: "This example shows a [funnel chart](/r/funnel-series/) built with AG Charts, showing how much of a total remains at each stage of an ordered sequence. A [formatter](/r/formatters/) renders the values as whole numbers, and a [tooltip renderer](/r/tooltips/#modifying-content) reports each stage's conversion.",
    },
    'customised-funnel': {
        ...
        intro: 'This example shows a [horizontal funnel chart](/r/funnel-series/#horizontal-funnel) built with AG Charts, reading each stage against a target as well as against the stage before it. An [item styler](/r/stylers/#item-stylers) raises the fill opacity of the stages that hit their target, the [drop-off areas](/r/funnel-series/#customisation) are faded, [stage labels](/r/funnel-series/#labels) sit before each bar, and the tooltip has a custom [position](/r/tooltips/#tooltip-position) to the left of the cursor falling back to the right where needed.',
    },
    ...
    'simple-pyramid': {
        ...
        intro: "This example shows a [pyramid chart](/r/pyramid-series/) built with AG Charts, showing how a population thins as it moves up a set of ordered tiers, with each tier's area carrying its share of the whole. The [aspect ratio](/r/pyramid-series/#shape) widens the triangle, [labels](/r/pyramid-series/#labels) print a share and value on the tiers that have space for them, and a [tooltip renderer](/r/tooltips/#modifying-content) names the open-ended top tier in full.",
    },
    ...
    'customised-radial-gauge': {
        ...
        intro: 'This example shows a [radial gauge chart](/r/radial-gauge/) built with AG Charts. The [start and end angles](/r/radial-gauge/#start-and-end-angles) open the sweep out well past a half-circle, and the scale is cut into [segments](/r/radial-gauge/#segmentation) exactly at the thresholds that matter rather than at even intervals. A triangular [target](/r/radial-gauge/#targets) marks the limit, a [corner radius](/r/radial-gauge/#corner-radius) rounds the bar, and a [tooltip renderer](/r/tooltips/#modifying-content) reports the current band.',
    },
    ...
    'horizontal-linear-gauge': {
        ...
        intro: 'This example shows a [horizontal linear gauge chart](/r/linear-gauge/#horizontal-linear-gauge) built with AG Charts, reading as a progress measure against a target. The bar is split into [segments](/r/linear-gauge/#segmentation), its [scale labels](/r/linear-gauge/#labels) are formatted as named stages rather than numbers, a circular [target](/r/linear-gauge/#targets) marks the goal, and a [tooltip renderer](/r/tooltips/#modifying-content) uses the tooltip to reports the gap to the target.',
    },
    'simple-bullet': {
    ...
    'step-interpolation-combination': {
        ...
        intro: 'This example shows a [combination chart](/r/combination-series/) built with AG Charts, combining a [range area series](/r/range-area-series/) with two [line series](/r/line-series/), all using [step interpolation](/r/line-series/#interpolation) so each value holds flat until the next period. A [grouped category axis](/r/axes-types/#grouped-category) nests the periods and rotates its inner labels, with the value axis pinned to fixed [interval values](/r/axes-intervals/#values).',
    },
    ...
    'stacked-bar-area-combination': {
        ...
        intro: "This example shows a [combination chart](/r/combination-series/) built with AG Charts. It shows an [area series](/r/area-series/) using [step interpolation](/r/area-series/#interpolation) over seven [stacked bar series](/r/bar-series/#stacked-bar), so a combination and an independent measure over the same period are read together. A [unit time axis](/r/axes-time/#unit-time) spaces the periods evenly, an [item styler](/r/stylers/#item-stylers) fades each bar's opacity by value, and the value axis sits on the right with a fixed [step interval](/r/axes-intervals/#step).",
    },
};
