...
import { SELF_PARITY, SELF_PARITY_PORTS } from './e2e/parity/targets';
...
// ports (PARITY_TARGETS) serves the ports itself and needs just the reference, unless
// PARITY_REFERENCE_URL points at a reference served elsewhere.
const webServer = SELF_PARITY
    ? [preview(SELF_PARITY_PORTS.reference), preview(SELF_PARITY_PORTS.port)]
    : process.env.PARITY_REFERENCE_URL
      ? []
      : [preview(SELF_PARITY_PORTS.reference)];
