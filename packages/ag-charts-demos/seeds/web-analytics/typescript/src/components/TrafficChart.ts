...
import { createAgChart } from '../agChart';
...
export interface TrafficChart extends View {
    /** Apply new props, as a re-render with them would. */
    update(props: TrafficChartProps): void;
}
...
const domainKeyOf = (daily: DailyPoint[]) =>
    daily.length > 0 ? `${daily[0].date.getTime()}:${daily.at(-1)!.date.getTime()}:${daily.length}` : '';

// The props the chart options are built from (the `useMemo` dependencies).
const OPTION_DEPS = [
    'metric',
    'daily',
    'dailyPrevious',
    ...
    'onAnnotationRemove',
    'onAddEventAt',
] as const satisfies readonly (keyof TrafficChartProps)[];

export function createTrafficChart(initial: TrafficChartProps): TrafficChart {
    let props = initial;
    ...
    const buildOptions = ({
        ...
        onAnnotationRemove,
        onAddEventAt,
    }: TrafficChartProps): AgCartesianChartOptions => {
        ...
        return {
            ...
            listeners: {
                selectionChange: ({ source }) => {
                    ...
                    onSelectionChange(selectionToDays(view.chart?.getSelection() ?? []));
                },
    ...
    const view = createAgChart(buildOptions(props));
    ...
    let mountedDomain = domainKeyOf(props.daily);
    function pruneSelection() {
        const domainKey = domainKeyOf(props.daily);
        if (mountedDomain === domainKey) return;
        mountedDomain = domainKey;
        const domainDays = new Set(props.daily.map((d) => dayKey(d.date)));
        props.onSelectionChange(props.selectedDays.filter((d) => domainDays.has(dayKey(d))));
    }
    ...
    return {
        ...
        // The effects run in the React order: the wrapper's `chart.update`, then the selection sync,
        // then the domain prune, each only when its dependencies changed.
        update(next) {
            const previous = props;
            props = next;
            if (OPTION_DEPS.some((key) => previous[key] !== next[key])) view.update(buildOptions(next));
            if (
                previous.selectedDays !== next.selectedDays ||
                previous.metric !== next.metric ||
                domainKeyOf(previous.daily) !== domainKeyOf(next.daily)
            ) {
                syncSelection();
            }
            pruneSelection();
