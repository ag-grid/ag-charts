...
// render the same elements, roles and data-state attributes. Two Radix behaviours reka-ui does not
// share are added here (see PORTING.md): a character typed on the closed trigger moves the value to
// the option it matches, and an option is `aria-selected` only while it is both the value and
// focused.
import {
    Label,
    SelectContent,
...
<template>
    <Label class="wa-labeled-select">
        ...
        <SelectRoot v-model="model" v-model:open="open">
            <SelectTrigger class="wa-btn wa-select-trigger" :aria-label="ariaLabel" @keydown="onTriggerKeydown">
                <SelectValue />
