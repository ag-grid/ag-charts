<script lang="ts">
import type { AgCartesianCrossLineOptions, AgSelectionItem } from 'ag-charts-community';

import { ANNOTATION_COLOR } from '../chartTheme';
import { fmtDate } from '../format';
import type { Annotation } from '../types';
import type { FormAnchor } from './EventForm.vue';
import { startOfDay } from './dateFilter';
...
import type { AgAreaSeriesOptions, AgCartesianChartOptions, AgContextMenuItem } from 'ag-charts-community';
...
const emit = defineEmits<{
    /** Raised when the user changes the selection on the chart. */
    selectionChange: [days: Date[]];
    /** Raised with the clicked annotation's id, or null when the click lands elsewhere. */
    annotationSelect: [annotationId: string | null];
    annotationRemove: [annotationId: string];
    /** Raised from the context menu to open the add-event form on a given day. */
    addEventAt: [date: Date, anchor?: FormAnchor];
}>();
...
const options = computed<AgCartesianChartOptions>(() => {
    const { metric, daily, dailyPrevious, annotations, selectedAnnotationId } = props;
        ...
        contextMenu: {
            getItems: ({ allShowOnParams, coordinates }) => {
                    ...
                    const items: AgContextMenuItem[] = [
                        {
                            ...
                            action: () => emit('annotationRemove', annotation.annotationId),
                        },
                    ];
                    return items;
                }
                ...
                const addItem: AgContextMenuItem = node
                    ? { showOn: 'series-node', label, action: (ev) => emit('addEventAt', day, anchorOf(ev.event)) }
                    : { showOn: 'series-area', label, action: (ev) => emit('addEventAt', day, anchorOf(ev.event)) };
                return [addItem];
            },
        },
        listeners: {
            selectionChange: ({ source }) => {
                ...
                emit('annotationSelect', null);
                emit('selectionChange', selectionToDays(chartInstance()?.getSelection() ?? []));
            },
            // Fires on every datum click, including a re-click that leaves the selection unchanged.
            seriesNodeClick: () => emit('annotationSelect', null),
            crossLineClick: ({ crossLineId }) => emit('annotationSelect', crossLineId),
            // Only an empty-area click reaches here; cross-line and datum clicks return before chart listeners.
            click: () => emit('annotationSelect', null),
        },
    };
});
...
// The wrapper creates the chart in its own mount, which runs before this one.
onMounted(syncSelection);
// Post-flush, so the wrapper's `options` watcher has pushed the new series first, as the React
// wrapper's effect runs before this component's.
watch([() => props.selectedDays, () => props.metric, domainKey], syncSelection, { flush: 'post' });
...
let mountedDomain = domainKey.value;
watch(
    domainKey,
    (key) => {
        if (mountedDomain === key) return;
        mountedDomain = key;
        const domainDays = new Set(props.daily.map((d) => dayKey(d.date)));
        emit(
            'selectionChange',
            props.selectedDays.filter((d) => domainDays.has(dayKey(d)))
        );
    },
    { flush: 'post' }
);
