...
import { WebAnalyticsDemo } from './index';
...
wrapper.setAttribute('data-demo-id', 'web-analytics');
wrapper.setAttribute('style', 'position: fixed; inset: 0;');
container.append(wrapper);

// Angular bootstraps onto the element matching WebAnalyticsDemo's selector: the wrapper above.
// eslint-disable-next-line no-console
bootstrapApplication(WebAnalyticsDemo).catch((error) => console.error(error));
