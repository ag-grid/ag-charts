...
// state attributes) and follows Radix's pointer and keyboard behaviour: the roving focus of the
// toggle group, and the select's typeahead, focus handling and dismissal, with the Angular CDK
// overlay holding the select's listbox.
import {
    CdkConnectedOverlay,
    CdkOverlayOrigin,
...
export class SelectItem {
    ...
    readonly textId = `fin-select-item-text-${nextSelectItemId++}`;
 ...
 * trigger, with the listbox in an overlay while open. Pointer and keyboard behaviour follows Radix:
...
/**
 * Radix RovingFocusGroup, as the toggle group uses it. The group is a tab stop (`tabindex="0"`)
