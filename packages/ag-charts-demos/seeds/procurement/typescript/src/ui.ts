 ...
 * a stop as well; keyboard entry into the group lands on the active item, or the last focused, or
 * the first, while a click focuses what was clicked. Arrows move focus between items and loop,
 * Home/End (and PageUp/PageDown) jump to the ends; a group with an orientation ignores the other
 * axis's arrows. Shift+Tab from an item takes the group out of the tab order until focus has left,
 * so it lands before the group rather than on it. Focus is moved on a timeout, as Radix does, so
 * the keydown finishes before the focus events run.
 */
