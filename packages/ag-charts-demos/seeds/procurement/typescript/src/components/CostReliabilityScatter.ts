...
export interface CostReliabilityScatter extends View {
    update(props: Omit<CostReliabilityScatterProps, 'onSelect'>): void;
}
 ...
 */
export function createCostReliabilityScatter({
    onSelect,
    ...props
}: CostReliabilityScatterProps): CostReliabilityScatter {
    const chart = chartHost<AgCartesianChartOptions<SupplierScorecard>>();

    // A supplier with no orders in scope has no price to draw, only a card.
    const drawn = memo((rows: SupplierScorecard[]) => rows.filter((row) => row.orderCount > 0));
    ...
    const options = memo(
        ...
        ): AgCartesianChartOptions<SupplierScorecard> => {
    ...
    const compute = (next: Omit<CostReliabilityScatterProps, 'onSelect'>) =>
        options(drawn(next.rows), next.supplierColors, next.selectedSupplierId);
