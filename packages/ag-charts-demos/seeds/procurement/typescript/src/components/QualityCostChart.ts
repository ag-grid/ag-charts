...
export interface QualityCostChart extends View {
    update(props: Omit<QualityCostChartProps, 'onSelect'>): void;
}
 ...
 */
export function createQualityCostChart({ onSelect, ...props }: QualityCostChartProps): QualityCostChart {
    const chart = chartHost<AgCartesianChartOptions<Row>>();

    const data = memo((rows: QualityCost[]): Row[] =>
        [...rows]
    ...
    const compute = (next: Omit<QualityCostChartProps, 'onSelect'>) =>
        options(data(next.rows), next.supplierColors, next.selectedSupplierId);
