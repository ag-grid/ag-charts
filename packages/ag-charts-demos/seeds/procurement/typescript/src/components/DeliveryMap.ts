...
export interface DeliveryMap extends View {
    update(props: Omit<DeliveryMapProps, 'onShipmentClick'>): void;
}
...
export function createDeliveryMap({ onShipmentClick, ...props }: DeliveryMapProps): DeliveryMap {
    const chart = chartHost<AgTopologyChartOptions<MapPoint>>();

    const points = memo((shipments: TrackedShipment[]) =>
        shipments.map<MapPoint>((shipment) => ({
    ...
    const options = memo(
        (points: MapPoint[], selectedShipmentId: string | undefined): AgTopologyChartOptions<MapPoint> => {
            ...
            const markerSeries = STATUS_DRAW_ORDER.map<AgMapMarkerSeriesOptions<MapPoint>>((status) => ({
                ...
                itemStyler: ({ datum }) =>
                    dimmed(datum.id)
                        ? { fillOpacity: 0.25, strokeOpacity: 0.25 }
    ...
    const compute = (next: Omit<DeliveryMapProps, 'onShipmentClick'>) =>
        options(points(next.shipments), next.selectedShipmentId);

    let current = props;
    return {
        el: chart.el,
        mount: () => chart.mount(compute(current)),
        update(next) {
            current = next;
            chart.update(compute(next));
        },
