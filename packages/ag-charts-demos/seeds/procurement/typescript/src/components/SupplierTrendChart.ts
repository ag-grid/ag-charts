...
export interface SupplierTrendChart extends View {
    update(props: SupplierTrendChartProps): void;
}
 ...
 */
export function createSupplierTrendChart(props: SupplierTrendChartProps): SupplierTrendChart {
    const chart = chartHost<AgCartesianChartOptions<Row>>();

    const spec = memo(metricSpec);

    const data = memo((trend: Map<string, SupplierTrendPoint[]>, spec: MetricSpec): Row[] => {
    ...
    const compute = (next: SupplierTrendChartProps) => {
        const metric = spec(next.metric);
        return options(
            data(next.trend, metric),
            next.supplierNames,
            next.supplierColors,
            next.selectedSupplierId,
            metric
        );
    };
