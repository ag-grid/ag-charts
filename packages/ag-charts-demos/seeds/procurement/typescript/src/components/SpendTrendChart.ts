...
export interface SpendTrendChart extends View {
    update(props: SpendTrendChartProps): void;
}
 ...
 */
export function createSpendTrendChart(props: SpendTrendChartProps): SpendTrendChart {
    const chart = chartHost<AgCartesianChartOptions<SpendTrendRow>>();
    ...
    const options = memo(
        (
            ...
            subcategories: string[],
            grain: SpendTrendGrain
        ): AgCartesianChartOptions<SpendTrendRow> => {
    ...
    return {
        el: chart.el,
        mount: () => chart.mount(options(current.rows, current.subcategories, current.grain)),
        update(next) {
            current = next;
            chart.update(options(next.rows, next.subcategories, next.grain));
        },
