<script lang="ts">
import { COMMODITY_INK, MATERIAL_INK, NEUTRAL, SUBCATEGORY_RAMP } from '../chartTheme';
...
const options = computed<AgStandaloneChartOptions<SpendNode>>(() => {
    const { supplierColors } = props;
    const series: AgSunburstSeriesOptions<SpendNode> = {
        ...
        tooltip: {
            renderer: ({ datum }) => {
                ...
                return {
                    title:
                        datum.path.length > 1 ? `${datum.path.slice(0, -1).join(' › ')} › ${datum.name}` : datum.name,
                    data:
                        datum.spend > 0
                            ? [
