...
// elements, classes, roles and data-* state attributes) and follows Radix's pointer and keyboard
// behaviour: the roving focus of the tabs list and the toggle group, and the select's typeahead,
// focus handling and dismissal, with the Angular CDK overlay holding the select's listbox.
import {
    CdkConnectedOverlay,
    CdkOverlayOrigin,
...
export class SelectItem {
    ...
    readonly textId = `pc-select-item-text-${nextSelectItemId++}`;
...
/**
 * Radix RovingFocusGroup, shared by the toggle group and the tabs list. The group is a tab stop (`tabindex="0"`)
...
/** One item of the toggle group: a tab stop while it is the one last focused. */
@Directive({
    selector: 'button[pcToggleItem]',
    host: {
        '[tabindex]': 'group.roving.tabStop() === element ? 0 : -1',
        '(focus)': 'group.roving.onItemFocus(element)',
    },
})
export class ToggleItem {
    readonly value = input.required<string>();
    readonly element = inject<ElementRef<HTMLButtonElement>>(ElementRef).nativeElement;
    protected readonly group = inject(ToggleGroup);
}

/**
 * Radix `ToggleGroup.Root type="single"`: a radiogroup of buttons where exactly one is on, with
 * roving focus along both axes (the React demo gives the group no orientation).
 */
@Component({
...
export class ToggleGroup {
    ...
    readonly roving = new RovingFocus({
        ...
        items: () => this.items().map((item) => item.element),
...
/**
 * Radix `Tabs.List`: a vertical, looping `role="tablist"` whose triggers are its content children,
 * with the roving focus above.
 */
@Directive({
 ...
 * primary mouse down or Enter/Space, linked to its panel by id. The tab last focused keeps the
 * roving tab stop.
 */
@Directive({
...
export class TabTrigger {
    ...
    protected readonly selected = computed(() => this.value() === this.tabs.value());
    ...
    protected onFocus(): void {
        this.list.roving.onItemFocus(this.element);
