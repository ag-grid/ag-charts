 ...
 */
@Component({
    selector: 'div[pcCostReliabilityScatter]',
...
export class CostReliabilityScatter {
    readonly rows = input.required<SupplierScorecard[]>();
    ...
    private readonly drawn = computed(() => this.rows().filter((row) => row.orderCount > 0));

    protected readonly options = computed<AgCartesianChartOptions<SupplierScorecard>>(() => {
        ...
        const series: AgBubbleSeriesOptions<SupplierScorecard> = {
            ...
            listeners: {
                seriesNodeClick: ({ datum }) => this.select.emit(datum.supplierId),
            },
        };
