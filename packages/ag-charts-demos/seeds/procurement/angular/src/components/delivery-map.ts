...
export class DeliveryMap {
    ...
    readonly shipmentClick = output<string>();

    private readonly points = computed<MapPoint[]>(() =>
    ...
    protected readonly options = computed<AgTopologyChartOptions<MapPoint>>(() => {
        ...
        const markerSeries = STATUS_DRAW_ORDER.map<AgMapMarkerSeriesOptions<MapPoint>>((status) => ({
            ...
            listeners: {
                seriesNodeClick: ({ datum }) => this.shipmentClick.emit(datum.id),
            },
        }));
