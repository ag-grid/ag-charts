...
description: Analyzes a Jira issue from a product and user/developer experience perspective to propose well-reasoned solutions.
---

# Jira Issue Product and UX/DX Analysis Prompt
...
1.  **Synthesize All Information**: Read through all the gathered information from the primary and referenced tickets.
...
4.  **Define Success Criteria and Constraints**: Create a definitive list of what a successful solution looks like from a user and developer standpoint.
    -   **User-Facing Success Criteria**: What must be true for the user to consider this problem solved? (e.g., "Users can complete the task in 3 steps instead of 5.")
    -   **Developer-Facing Success Criteria**: What must be true for a developer using the API to feel it's intuitive and powerful? (e.g., "The API for this feature should require minimal configuration for the most common use case.")
    -   **Product Constraints**: What are the business or product limitations? (e.g., "must align with our existing design system," "should not require a breaking API change in a minor release," "must be solvable within one quarter").

### Phase 3: Proposing Solutions

1.  **Brainstorm Potential Solutions**: Based on the synthesized problem space, brainstorm at least 2-3 distinct product or API-level solutions.
