...
-   **Good commit/branch**: The commit or branch where the issue is NOT present (e.g., `origin/b12.3.0`)
...
-   `bad=HEAD good=origin/b12.3.0 test="yarn nx test ag-charts-community --testPathPattern='legend.test.ts'"`
-   `HEAD origin/b12.3.0 "yarn nx test ag-charts-community --testPathPattern='legend.test.ts'"`
-   `"yarn nx test ag-charts-community --testPathPattern='legend.test.ts'"` (uses HEAD as bad, prompts for good)
...
2. **Validate test files exist (if test command references specific files):**
    ...
    # Example: if TEST_COMMAND contains "legend.test.ts", verify it exists
    # Note: This is optional but helpful for catching configuration errors
    ```
...
1. **Determine repository root (handles worktrees):**
    ...
    if echo "$GIT_DIR" | grep -q worktrees; then
        ...
        # Extract main worktree path (e.g., /path/to/.git/worktrees/SoZPt -> /path/to/.git)
        MAIN_GIT_DIR=$(echo "$GIT_DIR" | sed 's|/\.git/worktrees/.*|/.git|')
        # Try to get main worktree root if it exists
...
4. **Handle special cases:**
    ...
    - **Build errors (not test failures)**: If a commit has build errors that prevent tests from running, skip it:
        ...
        git bisect skip
        ```
    - **Test files don't exist**: If a test file doesn't exist in older commits (e.g., it was added later), modify the test script to handle this gracefully:
        ```bash
        # Update BISECT_SCRIPT to use --passWithNoTests or check file existence first
        ```
    - **Unexpected test failures**: If tests fail in an unexpected way (different error than the target issue), skip it:
        ```bash
        git bisect skip
        ```
    - **"No tests found" errors**: If Jest reports "No tests found" for a test file that should exist, this might indicate:
        - The test file was moved/renamed in a later commit
        - The package structure changed
        - The test pattern doesn't match the file location
          Consider skipping or adjusting the test command for that commit range.
...
-   **Worktree performance**: If using a worktree, be aware that builds may be slower if node_modules are shared or need to be resolved from the main worktree

### Worktree Considerations

-   Git worktrees are automatically detected and handled
-   The script will attempt to use node_modules from the main worktree if not found in the current worktree
-   Test scripts are created in `tmp/` directory relative to the repository root (works for both regular repos and worktrees)
-   If you encounter path issues, verify with `git rev-parse --show-toplevel` to confirm the repository root
...
TEST_COMMAND="yarn nx test ag-charts-community --testPathPattern='legend.test.ts' && \
              yarn nx test ag-charts-enterprise --testPathPattern='nightingale.test.ts'"
```

**Important:** When testing files in different packages, ensure:

-   Each package name matches the actual package structure (`ag-charts-community` vs `ag-charts-enterprise`)
-   Test files exist in the specified packages (use `find` or `grep` to verify)
-   Use `--passWithNoTests` flag if a test file might not exist in older commits:
    ```bash
    TEST_COMMAND="yarn nx test ag-charts-community --testPathPattern='legend.test.ts' --passWithNoTests && \
                  yarn nx test ag-charts-enterprise --testPathPattern='nightingale.test.ts' --passWithNoTests"
    ```

### Testing with Specific Test Names

```bash
TEST_COMMAND="yarn nx test ag-charts-community --testPathPattern='legend.test.ts' --testNamePattern='custom marker shapes'"
```
...
-   `/git/bisect bad=HEAD good=origin/b12.3.0 test="yarn nx test ag-charts-community --testPathPattern='legend.test.ts'"`
-   `/git/bisect HEAD origin/b12.3.0 "yarn nx test ag-charts-community --testPathPattern='legend.test.ts'"`
-   `/git/bisect "yarn nx test ag-charts-community --testPathPattern='legend.test.ts'"`
