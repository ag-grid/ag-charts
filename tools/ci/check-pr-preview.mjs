 ...
 *
 * Usage: node tools/ci/check-pr-preview.mjs <preview-dir>
...
for (const [packageName, { paths }] of Object.entries(manifest.packages ?? {})) {
    for (const [relativePath, target] of Object.entries(paths ?? {})) {
        ...
        // A path ending in `/` is a bare-specifier prefix; anything else is the package's own
        // entry point. AG Charts publishes only entry points today.
