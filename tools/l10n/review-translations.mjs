// Advisory semantic review of ag-charts-locale translation changes.
//
...
// (key sets, formatters, ${variables}) live in the package's vitest suite.
//
// Raw fetch is used deliberately: AG Charts forbids third-party dependencies and
// CI ships no Anthropic SDK, so a dependency-free HTTP call is the right fit here.
//
// Env:
...
const SRC_DIR = 'packages/ag-charts-locale/src';
const EN_US = `${SRC_DIR}/en-US.ts`;
...
async function main() {
    ...
    const system = [
        'You are reviewing translation changes for the ag-charts-locale package.',
        ...
        'Flag: mistranslations or wrong meaning; altered, missing, or mistranslated placeholders/formatters; inappropriate tone or register for chart UI / screen-reader text; obviously unnatural phrasing.',
    ...
    const report = [
        ...
        '',
        `Semantic review of changed translations in \`${SRC_DIR}\` against \`en-US.ts\`. This is advisory — structural completeness (keys, placeholders, formatters) is enforced by \`yarn nx test ag-charts-locale\`.`,
        '',
        review,
        '',
        '---',
