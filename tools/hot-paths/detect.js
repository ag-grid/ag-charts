...
const DEFERRED_CALLBACK_RE =
    /(addEventListener|addListener|removeEventListener|\.on|\.once|\.then|\.catch|\.finally|\.subscribe|setTimeout|setInterval|requestAnimationFrame|queueMicrotask|addManagedListener|addDestroyFunc)\s*\(/;
...
function insideLoop(lines, lineNo) {
    ...
    for (let i = lineNo - 2; i >= 0; i--) {
        ...
        // it, so the walk stops at one. Every other arrow stays transparent: a
        // predicate passed to a helper that calls it straight away (`dropLastWhile`,
        // a comparator, any of the array methods) really does run per element, and
