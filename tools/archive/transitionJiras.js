...
(async () => {
    ...
    switch (command) {
        case 'COMMITTED_TO_PENDING_RC':
            await transitionIssuesForFilter(COMMITTED_ISSUES_FILTER, 'MERGED');
            break;
        case 'PENDING_RC_TO_READY_TO_VERIFY':
            await transitionIssuesForFilter(PENDING_RC_ISSUES_FILTER, 'NEW RC');
            break;
        default:
            console.error(
                'Invalid option. Possible options are [COMMITTED_TO_PENDING_RC|PENDING_RC_TO_READY_TO_VERIFY]'
            );
    }
})();
